...
public abstract class Dependency : MonoBehaviour
{
    ...
    protected void Bind<T>( MonoBehaviour bindObgect, MonoBehaviour target) where T : class
    {
        if (target is IDependency<T>) (target as IDependency<T>).Construct(bindObgect as T);
    }
}
